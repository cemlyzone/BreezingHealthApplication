...
public class PieChart extends View {
	...
	private ArrayList<PiePartEntity> alPercentage = new ArrayList<PiePartEntity>();
	...
	public PieChart(Context context, AttributeSet attrs) {
		...
		iMargin = (int) fnGetRealPxFromDp(20);
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < iDataSize; i++) {
			...
			paintPieFill.setColor(getContext().getResources().getColor(alPercentage.get(i).getColor()));
			
			fEndAngle = alPercentage.get(i).getValue();
	...
	public boolean onTouchEvent(MotionEvent event) {
		if (isClickable()) {
			...
			for (int i = 0; i < iDataSize; i++) {
				fTotalPercent += alPercentage.get(i).getValue();
	...
	public void setAdapter(ArrayList<PiePartEntity> alPercentage) throws Exception {
		this.alPercentage = alPercentage;
		iDataSize = alPercentage.size();
		float fSum = 0;
		for (int i = 0; i < iDataSize; i++) {
			fSum+=alPercentage.get(i).getValue();
		}
		if (fSum!=100){
